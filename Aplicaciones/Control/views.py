...
#------------------------codigo pdf---------------
def generar_pdf_reporte(request, reporte_id=None):
    try:
        ...
        if reporte_id:
            # Si se especifica un ID, generar PDF para un solo reporte
            reporte = Reporte.objects.get(id=reporte_id)
            reportes = [reporte]
        else:
            # Si no hay ID, generar PDF para todos los reportes
            reportes = Reporte.objects.all()
        
        # Datos para la tabla
        data = [['ID', 'Estudiante', 'Clase', 'Calificaciones', 'Promedio', 'Estado']]
        
        for reporte in reportes:
            calificaciones = [f"{c.tarea}: {c.nota}" for c in reporte.calificaciones.all()]
            calificaciones_str = "\n".join(calificaciones)
            
            data.append([
                str(reporte.id),
                f"{reporte.estudiante.nombre} {reporte.estudiante.apellido}",
                reporte.clase.nombre,
                calificaciones_str,
                f"{reporte.promedio:.2f}",
                reporte.estado
            ])
        ...
        elements.append(Paragraph(f"Generado el: {fecha}", fecha_estilo))
        ...
        filename = f"reporte_calificaciones_{timezone.now().strftime('%Y%m%d_%H%M%S')}.pdf"
